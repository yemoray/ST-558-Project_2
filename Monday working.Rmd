...
```{r}
day_data_Train_sub <- day_data_Train %>% dplyr::select(-c(dteday,weekday,registered,casual,week_day,Season,Year,weather_situation,casual_format,registered_format,count_cat,atemp,workingday,holiday,weathersit))

day_data_Test_sub <- day_data_Test %>% dplyr::select(-c(dteday,weekday,registered,casual,week_day,Season,Year,weather_situation,count_cat,atemp,workingday,holiday,weathersit))   #The test data is subsetted as well
```
...
Lin_reg_1_predict <- predict(Lin_reg1_train, newdata = day_data_Test_sub)
...
The four models (2 linear regression models, one random forest model and one boosted tree model) are compared below:
```{r}
