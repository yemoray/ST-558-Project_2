...
* Homoscedasticity: Residuals are randomly distributed across the regression line (Can be checked using the Residual vs. Fitted value scatter plot. The plot must not have a discernable pattern)  
...
To select the candidate models, I first subset the dataset to include only variables of interest for the linear regression and ensemble models
```{r}
...
#Get the missclassification rate 
Lin_reg_1_predict_tbl <- data.frame(Lin_reg_1_predict, day_data_Test_sub_lm$count)
Lin_reg_1_misclass <- mean((Lin_reg_1_predict-day_data_Test_sub_lm$count)^2)
Lin_reg_1_misclass
```

#### Second linear regression Model  (Jiashu, please add your linear model here)

Then we decided to use the best subset method to select the variables for logistic regression. Notice that we create a new variable called `count_cat` which is a categorical one. For those total count greater than 5000, `count_cat=High`, and for those total count less than 5000, `count_cat=Low`.

As discussed previously about the `temperature` vs `feeling temperature`, we decided to drop `feeling temperature` first.

```{r}
log_full <- glm(count_cat ~ ., data = day_data_Train_sub %>% dplyr::select(-atemp), family = binomial)
log_step <- log_full %>% stepAIC(trace = F)
...
```{r}
#Predict the response variable using the test data to evaluate model performance
Lin_reg_2_pro <- predict(log_step, newdata = day_data_Test_sub, type = 'response')

Lin_reg_2_predict <- ifelse(Lin_reg_2_pro>0.5, 'Low', 'High')

#Get the missclassification rate 
Lin_reg_2_predict_tbl <- table(Lin_reg_2_predict, day_data_Test_sub$count_cat)
Lin_reg_2_misclass <- 1-(sum(diag(Lin_reg_2_predict_tbl))/sum(Lin_reg_2_predict_tbl))
```
...
>>>>>>> afb56b7bd123543db4159fbbd85a94a2bda6bdfa
