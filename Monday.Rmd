...
packages <- c("readr","ggplot2","dplyr","knitr", "caret", "tidyr","lubridate", "forcats","leaps", "regclass","MASS",'gbm')
...
Above shows the coefficients of the logistic regression selected by best subset method with `r length(coef(log_step))-1` variables.
...
For a small data set, we want to try boosted tree regression on order to aviod the problem of overfitting. We want to use the method **Gradient Boosting Machines**, which has lots of flexcibility and can be provided several hyperparameter tuning options that make the function fit very flexible. 

```{r,results='hide'}
boosted_train <- train(count ~., 
                 data= day_data_Train_sub, 
                 method='gbm', 
                 trControl=trctrl,
                 preProcess=c("center", "scale"))
boosted_predict <- predict(boosted_train, newdata = day_data_Test_sub)
```
...
print("Boosted rmse:")
print(rmse(predict(boosted_train$finalModel, day_data_Test_sub), day_data_Test_sub$count))
```
